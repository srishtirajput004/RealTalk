...
const addMembers=TryCatch(async(req,res,next)=>{
    ...
    const uniqueMembers=allNewMembers.filter((i)=>!chat.members.includes(i._id.toString())).map((i)=>i_id);
    ...
    const allUsersName=allNewMembers.map((i)=>i.name).join(","); // "a,v,b" aise names aajaenge saare
...
const leaveGroup=TryCatch(async(req,res,next)=>{
    ...
    //if creator wants to leave the group
    if(chat.creator.toString()==req.user.toString()){  

        //making random member the new creator in grp
...
const renameGroup=TryCatch(async(req,res,next)=>{
    ...
    return res.status(200).json({
        success:true,
        message:"Group named successfully",
    });
});
...
const deleteChat=TryCatch(async(req,res,next)=>{
    ...
    //here we delete all msngs and attachments from cloudinary
...
const getMessages=TryCatch(async(req,res,next)=>{
    ...
    const [messages,totalMessagesCount]=await Promise.all([
        ...
        .sort({createdAt:-1})   //sorted in desc order
        .skip(skip)
        .limit(resultPerPage)
