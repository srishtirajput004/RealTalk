...
const adminLogin=TryCatch(async(req,res,next)=>{
    ...
    .cookie("chattu-admin-token",token,{...cookieOptions,maxAge:1000*60*15,}).json({
        success:true,
        message:"Authenticated Successfully, Welcome Dude",
    });
...
const adminLogout=TryCatch(async(req,res,next)=>{
    ...
    return res.status(200)
    .cookie("chattu-admin-token","",{...cookieOptions,maxAge:0,}).json({
        success:true,
...
const allUsers=TryCatch(async(req,res)=>{
    ...
    const transformedData=await Promise.all(users.map(async({name,username,avatar,_id})=>{
...
const getDashboardStats=TryCatch(async(req,res)=>{
    ...
    last7DaysMessages.forEach((message)=>{
        const indexApprox=(today.getTime()-message.createdAt.getTime())/dayInMilliSec;
